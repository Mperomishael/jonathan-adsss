...
/** Admin prices only (USD). No hardcoded fallback. Returns undefined if unset. */
function adminPriceToDollars(raw: unknown): number | undefined {
  if (raw === null || raw === undefined || raw === '') return undefined
  const n = Number(raw)
  if (!Number.isFinite(n) || n < 0) return undefined
  // Legacy cents only for clearly old large integers (e.g. 5000)
...
export default function FanCardPage() {
    ...
    const apply = (data: any) => {
      ...
      setAdminTiers({
        regular: {
          ...
          price: adminPriceToDollars(src.regular?.price ?? data.price),
          label: src.regular?.label || 'Regular Fan',
        },
        gold: {
          enabled: src.gold?.enabled !== false,
          price: adminPriceToDollars(src.gold?.price),
          label: src.gold?.label || 'Gold Fan',
        },
        diamond: {
          enabled: src.diamond?.enabled !== false,
          price: adminPriceToDollars(src.diamond?.price),
  ...
  useEffect(() => {
    ...
    setAdminTiers({
      regular: {
        enabled: src.regular?.enabled !== false,
        price: adminPriceToDollars(src.regular?.price ?? fanCardSettings.price),
        label: src.regular?.label || 'Regular Fan',
      },
      gold: {
        enabled: src.gold?.enabled !== false,
  ...
  const activePrice = tiers[selectedTier].price // dollars | undefined
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-lg font-black mt-2" style={{ color: style.accent }}>
                      ${typeof tiers[id].price === 'number' ? tiers[id].price.toFixed(2) : '—'}
                    </p>
                  </button>
                )
              })}
            ...
            <>
              ...
              <section className="max-w-2xl mx-auto">
                {typeof activePrice === 'number' && activePrice > 0 ? (
                  ...
                  <p className="text-center text-yellow-500/80 text-sm py-6">
                    This tier has no price set yet. Configure it in Admin → Fan Card.
                  </p>
                )}
              </section>
            </>
            ...
            <section className="max-w-md mx-auto mt-10">
              <div className="rounded-xl border border-yellow-600/40 bg-yellow-950/20 p-5 text-center space-y-3">
                ...
                <p className="text-xl font-black" style={{ color: TIER_STYLE[nextUpgrade].accent }}>
                  ${typeof tiers[nextUpgrade].price === 'number' ? tiers[nextUpgrade].price.toFixed(2) : '—'}
                </p>
                <button
                  type="button"
                  onClick={() => {
